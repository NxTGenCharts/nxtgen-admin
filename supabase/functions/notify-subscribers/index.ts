...
type EmailKind =
  | 'published' | 'edited' | 'entry_triggered'
...
function resolveEmailKind(eventType: string, status: string): EmailKind {
  ...
  // Sent by the automatic monitoring engine (signal-monitor) the
  // moment a pending order's entry price is reached, or by the manual
...
const KIND_THEME: Record<EmailKind, KindTheme> = {
  ...
  entry_triggered: {
    label: 'Entry Triggered', accent: '#2563eb', accentBg: '#eff6ff', accentDark: '#60a5fa', accentBgDark: '#1e293b',
    subjectEmoji: '✅', headline: () => 'Entry Triggered'
  },
  tp1_hit: {
...
function buildSubject(kind: EmailKind, signal: SignalRow): string {
  ...
  switch (kind) {
    ...
    case 'entry_triggered': return `${emoji}Entry Triggered • ${signal.pair}`;
